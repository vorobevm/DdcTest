...
namespace ConsoleApplication3
{
    ...
    public static class DdcRoutinesStatic
    {
        ...
        {
            SqlCommand sql = new SqlCommand("insert into ddc.condition (StartDate, FinishDate, AmountPercent, AmountQty, ContractId, CreateDate, CreateUser, EditDate, EditUser, Status, AllBrandsSelected, AllProductsSelected, BusinessDomainId, IsDeleted) Values(cast(@StartDate as Datetime), cast(@FinishDate as Datetime), 100, 1, @ContractId, SYSDATETIME(), 'TEST', SYSDATETIME(), 'TEST', 1, 0, 0, 2, 0)", dbContext.GetConnection());
            sql.Parameters.AddWithValue("@StartDate", startDate.ToString());
            sql.Parameters.AddWithValue("@FinishDate", finishDate.ToString());
            sql.Parameters.AddWithValue("@ContractId", contractId);
            sql.ExecuteNonQuery();
